...
/**
 * UCSC genome-browser link for one alignment — the click-through the
 * legacy Gemma probe page had.
 *
 * The assembly is NOT `blatResult.targetDatabase`, which is a plain
 * label ("human"); it is `taxon.externalDatabase.name` ("hg38"), the
 * UCSC db name. Chromosome names arrive bare ("1", "6_GL000253v2_alt")
 * and UCSC wants the `chr` prefix, which is also correct for alt
 * contigs. Returns null when the assembly or the coordinates are
 * missing, so the row falls back to plain text rather than a link that
 * lands nowhere.
 */
function ucscUrl(b: NonNullable<GeneMappingSummary["blatResult"]>): string | null {
  ...
  // The alignment itself, as a custom track, so the probe is drawn
  // against the genome rather than the visitor landing on a locus with
  // nothing marked. Omitted when a PSL row can't be formed; the
  // position link still works.
  const track = pslTrack(b, seq);
  if (track) params.set("hgt.customText", track);
  return `https://genome.ucsc.edu/cgi-bin/hgTracks?${params.toString()}`;
}

/**
 * The probe alignment as a UCSC custom track.
 *
 * Ported field-for-field from `BlatResult2Psl.blatResult2PslTrack()` in
 * `gemma-core` — the Gemma 1.0 probe page built this server-side, and
 * that method still exists but has no callers since the ExtJS layer
 * went. Kept identical to it, including the 1000bp `browser position`
 * padding, the quoted query name, and space-separated PSL fields.
 *
 * `tSize` is the one field the REST payload doesn't carry. The Java
 * has the same gap and falls back to `targetEnd + 1`, commented "seems
 * okay as long as more than the target end" — same fallback here, for
 * the same reason.
 */
function pslTrack(
  ...
  return [
    "## Generated by Gemma",
    `browser position ${seq}:${Math.max(0, b.targetStart - pad)}-${b.targetEnd + pad}`,
    `track name="${q.name}" description="Gemma BLAT alignment" visibility=2 useScore=1`,
    psl,
    "",
  ].join("\n");
}
