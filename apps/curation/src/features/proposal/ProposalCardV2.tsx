...
export function ProposalCardV2({
  ...
  return (
    <div className="card">
      ...
      {/* ---------------- Downstream shape recommendations ----------------
          Surfaces split / subset-DEA / partial-coverage recommendations
          the gestalt landed on ``Design.should_split_on_factor_id`` and
          ``Design.subset_recommendations``. Read-only at this surface —
          the canonical accept/reject UI still lives in the design tab's
          "Experiment-wide decisions" pane (per Paul 2026-06-14: leave
          them in both places). The curator scanning a proposal needs
          to SEE these here so they don't miss the recommendation; the
          full disposition lives where it always lived. The factor
          involved gets surfaced with a chip strip of its FV labels so
          the curator can grok the shape inline without tab-switching. */}
      {(() => {
...
// ``StatementGlyph`` + ``GlyphTermRow`` were inlined here; both moved
// to ``@/components/ui/StatementGlyph.tsx`` so the audit panel can
// import them without duplicating the SVG / popover code.
