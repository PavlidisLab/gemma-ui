...
import { groupByReason, type OpenRow } from "./TriageCloseDialog";

const row = (targetId: number, label: string, reason?: string | null): OpenRow => ({
  targetId,
  label,
  reason: reason ?? null,
});
...
describe("groupByReason", () => {
  ...
  it("collapses reasonless rows into a single unlabelled bucket", () => {
    const groups = groupByReason([row(1, "GSE1"), row(2, "GSE2")]);
  ...
  it("treats whitespace-only reasons as no reason", () => {
    const groups = groupByReason([row(1, "GSE1", "   ")]);
    expect(groups[0].reason).toBeNull();
  });
