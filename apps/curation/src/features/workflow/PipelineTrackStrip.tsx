...
function badgeClass(status: StepStatus): string {
  switch (status) {
    ...
    case "incomplete":
    case "stale":          return "bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300 ring-amber-200 dark:ring-amber-800";
...
function statusSymbol(status: StepStatus): string {
  switch (status) {
    ...
    // Not "!" — the step is not in trouble, its answer has drifted from
    // the design it was computed against.
    case "stale":          return "~";
    case "na":              return "–";
    case "not_run":
    default:                return "○";
