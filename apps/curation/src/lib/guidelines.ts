...
export const CATEGORY_GUIDELINES: Record<string, GuidelineSnippet> = {
  ...
  "collection of material": {
    ...
    donts: [
      ...
      "Don't invent a `collection of material` factor purely to carry a subsetting hint when an existing factor already makes the split.",
      "There is no `study design: SUBSET` tag — never emit one. The recommendation rides as a single consolidated \"recommend subset DEA on factor X\".",
    ],
  },
  ...
  "disease model": {
    ...
    bullets: [
      ...
      "One statement per inducer — two inducing agents means two `induced by` statements on the one value.",
      "Default rule: animal taxon + abstract mentions `model` / `induced` / `transgenic` / `KO` → Disease model.",
    ],
    donts: [
      "Don't hang a dose on a disease model — `delivered at dose` statements here were dropped by ruling. A dose belongs to a treatment, and a disease model is not a treatment.",
      "Don't reach for `has modifier` when you mean causation — `induced by` is the causal predicate; `has modifier` is state.",
    ],
  },
  ...
  phenotype: {
    ...
    sourceUrl: WIKI_BASE,
    bullets: [
      "Drug-resistance pattern: `resistant to` (PATO_0001178) + `towards` (RO_0002503) + drug (CHEBI). Sensitive: PATO_0000516. Susceptible: PATO_0001152. Response: PATO_0000077.",
...
export const PREDICATE_GUIDELINE: GuidelineSnippet = {
  ...
  bullets: [
    ...
    "`has role` (RO_0000087) — the role the subject plays; mostly the baseline marker. object + has role + control / wild type genotype / initial time point / reference (substance|subject) role. Other roles sparingly: a chemical role (Compound-A + has role + antioxidant) or a sub-experiment name. 🛑 Never on a disease subject — a disease brought about by an agent is `induced by`.",
    ...
    "`delivered for duration` (TGEMO_00167) — how long a treatment or exposure ran: drug + delivered for duration + free-text time. Also for disease-progression timelines. 🛑 Not for an instantaneous event — a single injection, a surgery or an injury has no duration; the time since it is `sampled after`.",
    ...
    "`has modifier` (RO_0002573) — object differs from original form; a direction within a structure, grounded (Ammon's horn + has modifier + ventral EFO_0001662; left / right are PATO_0000366 / PATO_0000367), used only when the sub-region has no UBERON term of its own; or fallback when no other predicate fits. 🛑 A direction never takes `located in` (ruling, 2026-09-14), nor the relational `medial to` / `lateral to`.",
    ...
    "`located in` (RO_0001025) — disease or genotype manipulation in a place: a tissue or organ. Used inside genotype EFCs too. Not a direction within a structure (`has modifier`), not a gene in a cell type (`targeted to`).",
    ...
    "`has background` (TGEMO_00216) — the genetic background a line, strain or genotype sits on, for what no tag can say: a background that VARIES across the arms (Bmal1 knockout + has background + C57BL/6 on one arm, a second line on another background), a cell line's background, or an ungroundable strain hung off its grounded parent. 🛑 A strain background CONSTANT across every sample is a `strain background` tag (TGEMO_00217) with the same value, not this statement (ruling 2026-09-15) — and not a bare `strain` tag either, since a constant C57BL/6 on every sample of a knockout study is the background, not those animals' strain. The object is a grounded strain or cell line term. 🛑 Gemma stores two predicate/object pairs per statement and refuses a third on the same statement, so on a subject already carrying two — a compound genotype most often — the background needs its OWN statement.",
    "`has_allele` (GENO_0000413) — names WHICH allele: gene (NCBI) + has_allele + the allele (R521H, APOE4, CAG expansion mutation). A different assertion from `has_genotype`, whose objects are allele STATES; the two are independent statements about the same gene and are often both right. Not `has modifier`, which would erase which allele. 🛑 An allele with its own MGI term is the genotype VALUE instead, with zygosity as a statement on it — Gemma reaches a disease-model relation only from annotation values, never from a has_allele object.",
    "`has quality` (RO_0000086) — an intrinsic, observable attribute the subject HAS: light + has quality + blue. A role the subject PLAYS is `has role`; a genetic state such as overexpression is `has_genotype`. Its parent `has characteristic` (RO_0000053) is not offered: it is true of all four of its children (has quality, has role, has function, has disposition), so pick the child. If none fits, the statement is the wrong shape.",
    ...
    "`sampled after` (TGEMO_00202) — time from a treatment / disease event to sampling, with a reference subject in the experiment. Also the time since an instantaneous event (a single injection, a surgery, an injury), which has no duration. Distinct from `delivered for duration`, which is how long an exposure ran.",
  ],
  donts: [
    "Not offered: `has characteristic` (RO_0000053), the parent term — pick `has quality` or `has role` (Paul, 2026-09-13). Deprecated: `has biological role`, `is_gene_target_of`. Don't use.",
  ],
};
 ...
 * (the four terms, which category, the never-three, the host) and
...
/**
 * Statement templates — the composed annotation shapes.
 *
 * Re-derived from `gemma-curation-agents`
 * `docs/curation_rules/13_statement_templates.md`, which is the layer
 * ABOVE the predicate allow-list: `07_predicates.md` says what each
 * predicate means and when it's legal; that file says which whole
 * composed annotations we've settled on. The point is that a curator
 * finds the situation and copies the shape rather than re-deriving it
 * from the predicate list and landing on something plausible-but-wrong.
 *
 * Rendered beside the annotation editor (the StatementEditor's
 * predicate row) as well as in the Overview guidelines bar, per the
 * open ask recorded in that file's "Where these are enforced" table.
 *
 * The executable half of this lives in
 * `features/design/statementTemplates.ts` — the same shapes as pickable
 * `+ tpl…` entries. Keep the two in step: a shape added there wants a
 * line here, and vice versa.
 */
export const STATEMENT_TEMPLATE_GUIDELINE: GuidelineSnippet = {
  title: "Statement templates — the composed shapes",
  ...
  bullets: [
    ...
    "Which allele: `gene (NCBI_GENE) + has_allele (GENO_0000413) + the allele` for a point mutation, repeat expansion or lab-named allele. An allele with its own MGI term is the genotype VALUE instead, with zygosity as a statement on it.",
    "Treatment duration vs time since: `drug + delivered for duration + 6 h` is how long it ran; `<cell type | timepoint> + sampled after + 33 d` is the time from an event to sampling. An instantaneous event (a single injection, a surgery, an injury) has no duration. A 6-hour exposure sampled at 24 h is two statements.",
    "Deprivation: `decreased amount (PATO_0001997) + towards (RO_0002503) + <withheld substance>`; `absent` (PATO_0000462) only when the protocol states complete removal, and `starvation` (EFO_0022074) as the value when nutrition generally is withheld. The deprived arm is the perturbation; `reference substance role` goes on the replete arm.",
    "Onset: `disease (MONDO) + has modifier (RO_0002573) + <onset term (HP)>` — a quality of the disease, not a developmental stage. There is no onset predicate. Age at diagnosis is a number on a continuous `age at diagnosis` factor, and a bare `age` means age at sampling.",
    ...
    "…and hook them where the record allows: the name stays free text and the ontology enters on the object — `WTC-11 + derives from cell line cell + induced pluripotent stem cell line cell`. Most specific first: parent line, donor disease, tissue, origin cell type, and only last the line class. Never bind the name to the object's URI. For an experiment tag the hook is required — with nothing to ground the object, don't add the tag; a factor value or characteristic is not gated this way.",
  ...
  examples: [
    "cell type: astrocyte (CL_0000127) + derives from part of -> spinal cord (UBERON_0002240)",
    "cell type: retinal cell (CL_0009004) + derives from cell line cell -> H9 cell (CLO_0003612)",
