...
export const PREDICATE_GUIDELINE: GuidelineSnippet = {
  ...
  bullets: [
    ...
    "`has background` (TGEMO_00216) — the genetic background a line, strain or genotype sits on, when the background is constant and is not itself the property under study: Bmal1 knockout + has background + C57BL/6. Put it on the line or genotype, never as a bare strain annotation of the samples — a constant C57BL/6 on every sample of a knockout study is the background, not those animals' strain. The object is a grounded strain term. 🛑 Gemma stores two predicate/object pairs per statement and refuses a third on the same statement, so on a subject already carrying two — a compound genotype most often — the background needs its OWN statement.",
