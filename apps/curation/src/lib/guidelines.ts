...
export const PREDICATE_GUIDELINE: GuidelineSnippet = {
  ...
  bullets: [
    ...
    "`has_allele` (GENO_0000413) — names WHICH allele: gene (NCBI) + has_allele + the allele (R521H, APOE4, CAG expansion mutation). A different assertion from `has_genotype`, whose objects are allele STATES; the two are independent statements about the same gene and are often both right. Not `has modifier`, which would erase which allele.",
