 ...
 */
type GlyphTerm = { label?: string; uri?: string | null } | null | undefined;
type GlyphStatement = {
  subject?: GlyphTerm;
  predicate?: GlyphTerm;
  object?: GlyphTerm;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {statements.length > 1 ? (
          <span className="text-[10px] text-slate-500 leading-none ml-0.5">
            ×{statements.length}
          </span>
        ) : null}
      </button>
      {open && (
        ...
        >
          ...
          <ul className="space-y-1.5">
            {statements.map((s, i) => (
              <li
                key={i}
                className={`${
                  i > 0 ? "pt-1.5 border-t border-slate-100" : ""
                }`}
              >
          ...
          <div className="mt-2 pt-1.5 border-t border-slate-100 flex items-center gap-2 text-[10px] text-slate-500">
            <span className="inline-flex items-center gap-1">
...
function GlyphTermRow({
  ...
  return (
    <div className="flex items-baseline gap-1.5">
      <span className="font-mono text-[10px] text-slate-400 w-3 text-right">
        {ROLE_LABEL[role]}
      </span>
      {!term ? (
        ...
        <>
          ...
          {term.uri ? (
            <a
              ...
              className="font-mono text-[10px] text-slate-400 hover:text-blue-700 hover:underline"
              onClick={(e) => e.stopPropagation()}
              title={term.uri}
            >
              {shortenUri(term.uri)}
            ...
            <span className="text-[10px] text-slate-400">free-text</span>
          )}
        </>
      )}
    </div>
